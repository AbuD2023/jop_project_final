...
class OrderProvider extends ChangeNotifier {
  ...
  Future<void> getOrdersByCompanyIdWithAI({required int companyId}) async {
    try {
      _isLoading = true;
      _error = null;
      notifyListeners();
      ...
      // جلب البيانات من API
      final response =
          await _apiController.get<List<OrderRecommendationDtoModel>>(
        endpoint:
            'EnhancedJobRecommendation/GetRecommendedApplicantsByCompany2/$companyId',
        fromJson: (json) => (json is List)
            ? json
                .map((item) => OrderRecommendationDtoModel.fromJson(item))
                .toList()
            : json['items']
                .map((item) => OrderRecommendationDtoModel.fromJson(item))
                .toList(),
      ...
      if (response.isNotEmpty) {
        _ordersByCompany = response.map((order) => order.order!).toList();
        await getOrdersByCompanyId(companyId: companyId);
        notifyListeners();
  ...
  void searchOrderByCompany(String value, int companyId) {
    // String query = value;
    log(value);
    // if (value == 'الكل') {
    ...
    //   return;
    // }
    if (value == 'بنائاً على السيرة الذاتية') {
      getOrdersByCompanyIdWithAI(companyId: companyId);
      notifyListeners();
      return;
    }

    getOrdersByCompanyId(companyId: companyId);
    // notifyListeners();
  }
}
