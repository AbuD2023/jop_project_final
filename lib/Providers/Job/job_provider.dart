...
class JobsProvider with ChangeNotifier {
  ...
  Future<void> getJobsByAIs({required int searcherId}) async {
    if (searcherId != 0) {
      try {
        _isLoading = true;
        _error = null;
        ...
        if (response.isNotEmpty) {
          _jobsSearchPermanenceType = response.map((job) => job.job!).toList();
          // تخزين البيانات محلياً
          await prefs.setString(
              'jobs',
              json.encode(_jobsSearchPermanenceType
                  .map((jop) => jop.toJson())
                  .toList()));
        }
      } catch (e) {
        ...
        try {
          ...
          if (cachedData != null) {
            ...
            _jobsSearchPermanenceType = decodedData
  ...
  // Future<void> getJobsAIByCompanyId({required int companyId}) async {
  //   try {
  //     _isLoading = true;
  //     _error = null;
  //     notifyListeners();
  ...
  void searchPermanenceType(String value, int searcherId) {
    // String query = value;
    log(value);
    if (value == 'الكل') {
      getJobs();
    ...
    if (value == 'بنائاً على السيرة الذاتية') {
      getJobsByAIs(searcherId: searcherId);
      notifyListeners();
      return;
    }
    getJobs();
